...
@Component({
  ...
  template: `
	<div class="update-issue">
		...
		<form class="form" [formGroup]="form">
			<div class="form__col">
				<div class="form__row">
					...
					<input formControlName="title" type="text" [value]="issue.title" placeholder="Název title">
				</div>
				<div class="form__row">
					<span class="input-name">Adresa</span>
					<input formControlName="address" type="text" [(ngModel)]="address" placeholder="address" [attr.disabled]="true">
				</div>
			</div>
			...
			<div class="form__col">
				<div class="form__row">
					...
					<select #master_select formControlName="master">
						<option *ngFor="let master of masters"
							[value]="master.name">
							{{ master.name }}
						</option>
					</select>
		...
		<div class="buttons">
			<div>
				<button (click)="archiveIssue()"
					class="btn yellow-btn btn_small archive-button"
					[ngClass]="{
						'danger-btn': isArchived,
						'success-btn': !isArchived
					}">
					<i class="fas fa-box-open"></i>
				</button>
				<span *ngIf="isArchived" class="secondary-text">Závada je archivována. Klikněte pro zrušení.</span>
			</div>
			<div>
				<button #close_btn class="btn black-btn btn_small">Zavřít</button>
				<button #update_btn [disabled]="!form.valid" class="btn yellow-btn btn_small">Uložit</button>
			</div>
		</div>
	</div>
  `,
  styleUrls: ['./update-issue.component.sass']
})
export class UpdateIssueComponent implements OnInit, AfterViewInit {
	...
	ngOnInit(): void {
		this.masterService.getAllMasters().subscribe(
			...
			() => {
				setTimeout(() => {
					let res = this.masters.map(master => master.name).indexOf(this.issue.master);
	...
	ngAfterViewInit(): void {
		...
		this.mapbox.addDraggableMarker(location);
